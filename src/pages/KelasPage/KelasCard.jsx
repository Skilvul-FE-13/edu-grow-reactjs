...
const KelasCard = () => {
    ...
    return (
        <Container>
            <div className="mx-0 my-12 grid grid-cols-1 gap-4 max-[350px]:mx-4 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-3">
                {kelasList.map((kelas) => (
                    <Card
                        key={kelas.slug}
                    ...
                    >
                        ...
                        <Link
                            to={`/kelas/${kelas.slug}`}
                            className="dark:bg-cyan-600 dark:hover:bg-cyan-700 dark:focus:ring-cyan-800 rounded-lg bg-[#3A5088] px-5 py-2.5 text-center text-sm font-medium text-white hover:bg-[#C8D4E4] hover:text-black focus:outline-none focus:ring-4 focus:ring-cyan-300"
                        >
                            Pilih
                        </Link>
                    </Card>
